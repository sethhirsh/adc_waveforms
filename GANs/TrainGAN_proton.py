...
Gx = Dropout(0.3)(Gx)
Gx = Dense(100, activation="tanh")(Gx)
Gx = Dense(real_p_adc.shape[1], activation="tanh",kernel_initializer=RandomUniform(-0.01,0.01), bias_initializer='zeros')(Gx)
...
#D.set_weights(load_model('GAN_disc_penalize_150.h5').get_weights())
#G.set_weights(load_model('GAN_gen_penalize_150.h5').get_weights())
#A.set_weights(load_model('GAN_GAN_penalize_150.h5').get_weights())
...
#D.fit(mixed,labels,epochs=5)
#D.fit(real_p_adc,np.ones(len(real_p_adc)),epochs=1)
...
for i in range(epochs):   
    ...
    far_idxs=np.where(abs(mean_dist_full)>150)[0]
    near_idxs=np.where(abs(mean_dist_full)<150)[0]
    
    #Set proportion of each
    b_far=int(batch_size*0.3)
    b_near=int(batch_size*0.7)
    ...
    #squares
    if(np.mean(abs(fake1-fake2)) < 0.01):
        for i in range(3):
...
#Save models
#D.save('GAN_disc_penalize_100win_167.h5')
#G.save('GAN_gen_penalize_100win_167.h5')
#A.save('GAN_GAN_penalize_100win_167.h5')